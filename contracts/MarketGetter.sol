...
contract MarketGetter {
    ...
    /// @notice Get minimum guaranteed frame's award amount (accumulation of lot taxes)
    function getRewardAmountMin(Market market, uint frameKey) external view returns (uint rewardFund) {
        ...
        if (frameIndex > 0) {
            ...
            totalReward = ((prevFrame.rewardPool / 2) + frame.rewardPool) / 2;
        } else {
            totalReward =  (frame.rewardPool / 2);
        }

        // Correct Q96-based fee calculation
        uint fee = mulDiv(totalReward, market.feeProtocol(), FixedPoint96.Q96);
        rewardFund = totalReward - fee;
