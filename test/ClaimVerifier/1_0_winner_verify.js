...
describe('Settle frame and reward winning lot', function () {
  ...
  it('Settle frame', async function () {
    ...
    const rewardQ96 = await contractMarket.clcRewardFund(frameKey);
    const feeProtocol = await contractMarket.feeProtocol(); // already in Q96
    const feeQ96 = rewardQ96.mul(feeProtocol).div(Q96); // Q96 result
    const payoutQ96 = rewardQ96.sub(feeQ96);
  ...
  it('Deploy ClaimeVerifier contract and call claim', async function () {
    const ClaimVerifier = await ethers.getContractFactory('ClaimVerifier');
    claimVerifier = await ClaimVerifier.deploy(contractMarket.address);

    // call claim function
    await claimVerifier.connect(user).claim(frameKey);

    // Get the public mapping of the claimVerifier contract
    // mapping(address => mapping(uint => bool)) public hasClaimed;
    const hasClaimed = await claimVerifier.hasClaimed(user.address, frameKey);
    console.log('Has claimed:', hasClaimed);
  });
  it('Non winner calls claim function', async function () {
    // call claim function with non winner and expect revert
    const nonWinner = accounts[2];
    await expect(claimVerifier.connect(nonWinner).claim(frameKey)).to.be.revertedWith('Not the winner');
  });
});
