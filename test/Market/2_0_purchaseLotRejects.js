...
/*
Random user tries to buy a random lot but fails due to different reasons: tax <= 0, allowance is set too low, lot already exists
*/

describe('Purchase one lot but fails', function () {
  ...
  it('Purchase lot but fails', async function () {
    ...
    await expect(
      contractMarket
        .connect(user)
        .tradeLot(frameKey, pairPrice, 0, { maxFeePerGas: ethers.BigNumber.from(Math.floor(1.25 * block.baseFeePerGas)) })
    ...
    await expect(
      contractMarket
        .connect(user)
        .tradeLot(frameKey - 2700000, pairPrice, acqPrice, { maxFeePerGas: ethers.BigNumber.from(Math.floor(1.25 * block.baseFeePerGas)) })
    ...
    await expect(
      contractMarket
        .connect(user)
        .tradeLot(frameKey, pairPrice, acqPrice + 100, { maxFeePerGas: ethers.BigNumber.from(Math.floor(1.25 * block.baseFeePerGas)) })
