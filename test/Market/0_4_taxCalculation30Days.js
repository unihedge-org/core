...
describe('Tax Calculation for 30 Days with Fixed Price', function () {
  let accounts, owner, token, contractMarket, tokenDecimals, baseUnit, Q96, priceQ96;
  ...
  it('Test tax calculation for 30 days with price 10 USDC', async function () {
    ...
    const expectedValues = [
      { day: 1, percentage: 25.0, tax: 2.5 },
      { day: 2, percentage: 17.6777, tax: 1.7678 },
      { day: 3, percentage: 14.4338, tax: 1.4434 },
      { day: 4, percentage: 12.5, tax: 1.25 },
      { day: 5, percentage: 11.1803, tax: 1.118 },
    ...
    for (let day = 1; day <= 30; day++) {
      // Create frames at different daily intervals to ensure unique frame keys
      // Day 1: frame starts 1 day in future, ends 2 days in future
      // Day 2: frame starts 2 days in future, ends 3 days in future
      // etc.

      const daysInFuture = day;
      const frameStartTime = block.timestamp + daysInFuture * 86400;
      const frameKey = await contractMarket.clcFrameKey(frameStartTime);

      // Now simulate being at different times relative to this frame
      // to get the desired "days remaining" for each test

      // For day 1: we want 0 days remaining (< 24h left)
      // For day 2: we want 1 day remaining
      // For day 3: we want 2 days remaining
      const targetDaysRemaining = day - 1;
      const targetSecondsRemaining = targetDaysRemaining * 86400 + 3600; // +1 hour buffer

      // Calculate what current time should be to achieve target remaining time
      const frameEndTime = frameKey + 86400; // settlement time
      const simulatedCurrentTime = frameEndTime - targetSecondsRemaining;

      // Set the blockchain time to our simulated time
      await time.setNextBlockTimestamp(simulatedCurrentTime);

      console.log(`\x1b[90m   Day ${day}: Frame at ${frameKey}, simulating ${targetDaysRemaining} days remaining\x1b[0m`);
      ...
      console.log(
        '\x1b[32m%s\x1b[0m',
        `   Day ${day.toString().padStart(2)}: ${expectedPercentage.toFixed(4)}% -> Tax: ${taxFormatted.toFixed(
          4
        )} USDC (expected: ${expectedTaxAmount.toFixed(4)})`
      );
      ...
      expect(taxFormatted).to.be.closeTo(
        expectedTaxAmount,
        tolerance,
      ...
      if (day <= 8) {
        ...
        expect(taxFormatted).to.be.closeTo(expected.tax, 0.01, `Day ${day}: Should match expected value ${expected.tax}`);
      }
    }
  });
});
