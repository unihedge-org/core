...
describe("Resale lot", function () {
    let accounts, owner, user, user2, token, wPOLContract, contractMarket, swapRouter, frameKey, dPrice ,acqPrice, tax, rateAtStart, tokenDecimals;
    ...
    it('It should revert if not owner', async function () {
        ...
        //Change frame price with overrideFrameRate(Frame memory frame, uint avgPrice)
        await expect(contractMarket.connect(accounts[1]).overrideFrameRate(frameKey, 123456789)).to.be.revertedWith("Only owner can update frame close price");
    });
