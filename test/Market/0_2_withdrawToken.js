...
describe("Resale lot", function () {
    ...
    it('Owner withdraws funds from the contract', async function () {
        // Check contract's DAI balance
        let balanceBefore = await daiContract.balanceOf(contractMarket.address);
        console.log("\x1b[33m%s\x1b[0m", "   Contract DAI balance: ", ethers.utils.formatUnits(balanceBefore, 18), " DAI");
        // Check owner's DAI balance
        let ownerBalanceBefore = await daiContract.balanceOf(owner.address);

        // Owner withdraws funds from the contract
        await contractMarket.withdrawAccountingToken(balanceBefore);

        // Check contract's DAI balance
        let balanceAfter = await daiContract.balanceOf(contractMarket.address);
        console.log("\x1b[33m%s\x1b[0m", "   Contract DAI balance: ", ethers.utils.formatUnits(balanceAfter, 18), " DAI");
        // Check owner's DAI balance
        let ownerBalanceAfter = await daiContract.balanceOf(owner.address);
        console.log("\x1b[33m%s\x1b[0m", "   Owner DAI balance: ", ethers.utils.formatUnits(ownerBalanceAfter, 18), " DAI");

        expect(balanceAfter).to.equal(0);
        expect(ownerBalanceAfter).to.equal(ownerBalanceBefore.add(balanceBefore));
