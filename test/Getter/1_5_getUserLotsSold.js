...
describe("Get multiple users comulative awards", function () {
    ...
    it('get users lots sold', async function () {
        ...
        let lots = await contractMarketGetter.getLotsSoldUser(contractMarket.address, user.address, 0, frameKeyStart, frameKeyEnd, 0, 10);
        //filter non empty lots
        lots = lots[0].filter(lot => lot.lotKey != 0);
        console.log(lots)
    });
})
