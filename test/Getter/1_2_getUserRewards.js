...
*/
describe("Get get comulative awards", function () {
    let accounts, owner, user, user2, user3, daiContract, wMaticContract, contractMarket, contractMarketGetter, frameKeyStart, frameKey, dPrice ,acqPrice, acqPrice2, tax;
    ...
    it('second user buys lot', async function () {
        ...
        //Purchase lot 
        await contractMarket.connect(user2).tradeLot(frameKey, pairPrice, acqPrice, user.address, 
            {maxFeePerGas: ethers.BigNumber.from(Math.floor(1.25 * block.baseFeePerGas))}
        );
    ...
    it('Third user buys lot', async function () {
        ...
        //Purchase lot 
        await contractMarket.connect(user3).tradeLot(frameKey, pairPrice2, acqPrice2, user.address, 
            {maxFeePerGas: ethers.BigNumber.from(Math.floor(1.25 * block.baseFeePerGas))}
        );
        ...
        expect(user3ReferredBy).to.equal(user.address);
    ...
    it('Calculate users reward fund', async function () {
        ...
        let rewardFund = await contractMarketGetter.getUserRewards(contractMarket.address, user.address, frameKeyStart, frameKeyEnd);
