...
describe("Get lots by index", function () {
  ...
  it("Deploy Market and Getter contract", async function () {
    ...
    expect(contractMarketGetter.address).to.not.equal(
  ...
  it("Approve DAI to spend", async function () {
    ...
    console.log(
      "\x1b[36m%s\x1b[0m",
      "   Tax: ",
      ethers.utils.formatUnits(tax, 18),
      " DAI"
    );
  ...
  it("Purchase lot", async function () {
    ...
    await contractMarket
      .connect(user)
      .tradeLot(frameKey, pairPrice, acqPrice, {
        maxFeePerGas: ethers.BigNumber.from(
          Math.floor(1.25 * block.baseFeePerGas)
        ),
      });
  ...
  it("second user approves dai for purchase", async function () {
    ...
    await daiContract
      .connect(user2)
      .approve(contractMarket.address, tax.add(acquisitionPrice));
    //Check allowance
    let allowance = await daiContract.allowance(
      user2.address,
      contractMarket.address
  ...
  it("second user buys lot", async function () {
    ...
    expect(lotStates[2].taxCharged).to.equal(
      balanceBefore.sub(balanceAfter).sub(lotStates[1].acquisitionPrice)
    );
  ...
  it("Third user buys lot", async function () {
    ...
    await contractMarket
      .connect(user3)
      .tradeLot(frameKey, pairPrice2, acqPrice2, {
        maxFeePerGas: ethers.BigNumber.from(
          Math.floor(1.25 * block.baseFeePerGas)
        ),
      });
  ...
  it("Skip time to end of frame", async function () {
    ...
    console.log(
      "\x1b[33m%s\x1b[0m",
      "   Time difference: ",
      (block2.timestamp - block.timestamp) / 86400,
      " days"
    );
  ...
  it("get frames by index", async function () {
    ...
    let frames = await contractMarketGetter.getFramesByIndex(
      contractMarket.address,
      0,
      10
    );
